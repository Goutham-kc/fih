...
const KNOWN_MODULES = ['todo', 'done', 'debt', 'deadline', 'date', 'watch', 'list', 'mode', 'help'];
...
export function parseCommand(raw) {
  ...
  if (exact === 'mode') {
    const subTokens = rest.split(/\s+/);
    const targetModeRaw = subTokens[0]?.toLowerCase();
    const password = subTokens.slice(1).join(' ').trim();
    
    if (['dev', 'development', 'live'].includes(targetModeRaw)) {
      const targetMode = targetModeRaw === 'dev' ? 'development' : targetModeRaw;
      if (!password) {
        return { type: 'missing', module: 'mode', missingField: 'password',
          question: `Please provide your password to switch to ${targetMode.toUpperCase()} mode. (e.g. \`>mode ${targetModeRaw} yourpassword\`)`, partialData: { targetMode } };
      }
      return { type: 'mode', targetMode, password };
    }
