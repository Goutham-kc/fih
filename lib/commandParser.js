...
const KNOWN_MODULES = ['todo', 'done', 'debt', 'deadline', 'date', 'watch', 'list', 'help'];
...
export function parseCommand(raw) {
  ...
  // Handle text with or without leading >
  const body = text.startsWith('>') ? text.slice(1).trim() : text.trim();
  if (!body || ['hi', 'hello', 'hey', 'help', 'start'].includes(body.toLowerCase())) {
    return { type: 'help' };
  }
  ...
  if (exact === 'debt') {
    ...
    if (!['owe', 'owed', 'settle'].includes(action)) {
      return { type: 'missing', module: 'debt', missingField: 'action',
        question: "Did you mean `>debt owe`, `>debt owed`, or `>debt settle`?",
        partialData: {} };
    }
    if (action === 'settle') {
  ...
  if (exact === 'deadline') {
    ...
    const validCategories = ['academic', 'internship', 'personal'];
