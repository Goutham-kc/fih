...
export function parseNaturalLanguage(rawText) {
  ...
  const isDeadlineKeyword = /\b(due|deadline|before|by|schedule|submit|submission|finish|complete|evaluation|exam|viva|test|quiz|presentation|project|slides|hard\s+copy|report|assignment|notes|diary|manual|ppt|homework|lab|code|draft)\b/i.test(text);
