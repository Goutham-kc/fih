...
function parseTargetTime(text) {
  ...
  const offsetDiffMs = (330 + localOffsetMin) * 60 * 1000;
  
  // Shift reference date so its local getters return components matching Asia/Kolkata (IST)
  const refDate = new Date(now.getTime() + offsetDiffMs);
  ...
  if (results && results.length > 0) {
    ...
    // Shift the parsed date back to get the correct UTC instant
    parsedDate = new Date(parsedDate.getTime() - offsetDiffMs);
...
export function parseNaturalLanguage(rawText) {
  ...
  const watchMatch = text.match(/^(?:(?:wanna|want\s+to|plan\s+to|need\s+to|have\s+to|gotta|must)\s+)?(?:continue\s+)?(?:watch|watching|movie|show|anime|read|reading|binge|binging|stream|streaming)\s+(.+)/i) ||
                      ...
                      text.match(/^(.+?)\s+(?:to\s+)?(watch|read|binge|stream)$/i);
  if (watchMatch) {
