...
export function parseNaturalLanguage(rawText) {
  ...
  const watchMatch = text.match(/^(?:(?:wanna|want\s+to|plan\s+to|need\s+to|have\s+to|gotta|must)\s+)?(?:continue\s+)?(?:watch|watching|movie|show|anime|read|reading|book|paper|binge|binging|stream|streaming)\s+(.+)/i) ||
  ...
  if (watchMatch) {
    ...
    const isBook = /\b(read|reading|book|paper)\b/i.test(text);
