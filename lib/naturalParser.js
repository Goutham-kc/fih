...
export function parseNaturalLanguage(rawText) {
  ...
  if (hasDateContext && (isDeadlineKeyword || relativeMatch || dayMatch)) {
    let targetDate = null;
    ...
    if (relativeMatch) {
      ...
      const fullIsoMatch = text.match(/\b(\d{4})-(\d{2})-(\d{2})\b/);
      if (fullIsoMatch) {
        const year = parseInt(fullIsoMatch[1], 10);
        const monthIndex = parseInt(fullIsoMatch[2], 10) - 1;
  ...
  const isReminderKeyword = /\b(class|meeting|meet|lecture|session|conducted|hr|hour|today|tomorrow|tmr|tmrw|2moro|tonight|at\s+\d{1,2}|am|pm|remind|ticket|book|doctor|appointment|flight|train|bus|interview|standup|sync|webinar|workshop)\b/i.test(text);
  if (isReminderKeyword) {
    const remindAt = parseTargetTime(text);
    return { type: 'reminder', title: cleanTitle(rawClean.split('.')[0]), remindAt, confidence: 'high' };
  }

  // ── 5. TO-DO ACTION VERBS & PHRASES ─────────────────────────────────────
  ...
  const watchMatch = text.match(/^(?:watch|movie|show|anime|read|book|paper|binge|stream|wanna\s+watch|want\s+to\s+watch|plan\s+to\s+read|reading)\s+(.+)/i) ||
                      text.match(/^add\s+(.+?)\s+to\s+(?:my\s+)?watchlist\b/i);
  if (watchMatch) {
    const itemTitle = watchMatch[1].trim();
    const isBook = /\b(read|book|paper)\b/i.test(text);
    return { type: 'watch', title: itemTitle, watchType: isBook ? 'book' : 'show', confidence: 'high' };
  }
