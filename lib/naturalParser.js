...
export function parseNaturalLanguage(rawText) {
  ...
  if (watchMatch) {
    ...
    const isAnime = /\b(anime)\b/i.test(text);
    const isMovie = /\b(movie|film|binge|cinema)\b/i.test(text);
    const watchType = isBook ? 'book' : isAnime ? 'anime' : isMovie ? 'movie' : 'show';
    return { type: 'watch', watchAction: 'add', title: itemTitle, watchType, confidence: 'high' };
  }
